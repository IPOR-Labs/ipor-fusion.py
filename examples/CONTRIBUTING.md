...
Primary audience: these examples are read first by coding agents, then by humans. The two rarely
conflict, but when a formatting choice trades machine-robustness for human visual polish -- e.g.
ASCII-aligned columns or tables inside comments, whose alignment carries no signal to a tokenizer
and silently rots on the next edit -- prefer the agent-parsable form: one self-contained statement
per line, no reliance on 2D layout.
