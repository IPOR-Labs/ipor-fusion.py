...
#               balances (update_markets_balances), and nothing else --
#               least-privilege, it cannot reconfigure the vault. Collapsing it
#               into OWNER means a compromised alpha key becomes full governance
#               control.
...
def run_simulation(web3: Web3) -> SimulationResult:
    ...
    # notes above): a compromised alpha key must not equal governance. Each grant
    # uses Period(0) (no execution delay) to keep the simulation legible;
    # production governance often puts a nonzero delay -- a timelock -- on the
    # sensitive roles.
    sim.add_call(
    ...
    # Read the granted substrate set back so we can assert it is EXACTLY USDC.
    sim.observe("granted_substrates", plasma_vault.get_market_substrates(AAVE_MARKET))
    ...
    # Fund the depositor, then deposit into the vault. The whale -> depositor
    # transfer is SIMULATION ONLY (impersonated, validation disabled) and has NO
    # production equivalent -- a real depositor already holds their USDC. The
    # approve + deposit that follow are exactly what a real depositor signs.
    ...
    # No unrelated permissions: the granted set is EXACTLY the USDC substrate, and
    # nothing else. Compare as sets (grant order is not guaranteed).
    granted = {bytes(s) for s in result.get("granted_substrates")}
    _check(
        granted == {_address_substrate(BASE_USDC)},
